...
export class ArenaRoom {
  ...
  addPlayer(clientId, name, userId = null, skinId = 'default') {
    ...
    player.dead = false; // Players start alive in arena
    ...
    // Send room status immediately to all players including the new one
    this.broadcastRoomStatus();
    
    // Also send directly to the joining player to ensure they get it
    this.sendToClient(clientId, {
  ...
  startGame() {
    ...
    const BROADCAST_RATE = 10; // 10fps is enough for arena updates
  ...
  sendStateUpdate() {
    // Only send players that are alive to reduce packet size
    const alivePlayers = [];
    this.players.forEach(p => {
      if (!p.dead) {
